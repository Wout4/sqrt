...
struct interval *double_mult(struct interval *first, struct interval *second){
    ...
    struct interval *result = malloc(sizeof(struct interval));
    if (result == 0) { abort(); }
    result->a = nextafter(first->a * second->a,-INFINITY);
    result->b = nextafter(first->b * second->b,INFINITY);
    return result;
}
...
struct interval *double_div(struct interval *first, struct interval *second){
    ...
    struct interval *result = malloc(sizeof(struct interval));
    if (result == 0) { abort(); }
    result->a = nextafter(first->a / second->b, -INFINITY);
    result->b = nextafter(first->b / second->a, INFINITY);
    return result;
}
